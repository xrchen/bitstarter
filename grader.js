...
if(require.main == module) {
    program
        ...
        .option('-u, --url <url>', 'Url to web page', null, 'http://secret-badlands-6223.herokuapp.com/')
        .parse(process.argv);
    var checkJson, outJson;
    if (program.url) {
	rest.get(program.url).on('complete', function(result, response) {
	    fs.writeFileSync(TMP_HTML_FILE, result);
	    checkFile(TMP_HTML_FILE, program.checks);
	});
    } else {
